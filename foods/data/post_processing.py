...
def RNN_prediction(classifier, df):
    ...
    cc_svm = pd.Series(confusion_matrix(df['label'],  df['predict']).reshape(1,-1)[0], index=['tn', 'fp', 'fn', 'tp'])
    print('\n',cc_svm,'\n')
    print('sensitivity', (cc_svm['tp']/(cc_svm['tp'] + cc_svm['fn']).round(3)))
    print('precision', (cc_svm['tp']/(cc_svm['tp'] + cc_svm['fp'])).round(3))
    print('accuracy', ((cc_svm['tp'] + cc_svm['tn'])/(cc_svm['tp'] + cc_svm['fp']+cc_svm['fn'] + cc_svm['tn'])).round(3))
